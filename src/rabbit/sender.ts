...
async function connect(data: IMessage) {
    ...
    await channel.assertExchange(`email-${data.moduleName}`, 'direct', {durable: false});
}
...
export async function send(data: IMessage) {
    ...
    channel.publish(`email-${data.moduleName}`, `email-${data.moduleName}`, Buffer.from(JSON.stringify(data)));
